...
public class Gurty extends WaterMobEntity implements IMob {
	...
	static class Navigator extends SwimmerPathNavigator {
		Navigator(Gurty gurty, World worldIn) {
			super(gurty, worldIn);
		}
	...
	private static final DataParameter<Integer> TARGET_ENTITY = EntityDataManager.defineId(Gurty.class,
			DataSerializers.INT);
	
	private static final DataParameter<Boolean> ANGRY = EntityDataManager.defineId(Gurty.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> GOING_NEST = EntityDataManager.defineId(Gurty.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<BlockPos> NEST_POS = EntityDataManager.defineId(Gurty.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<BlockPos> TRAVEL_POS = EntityDataManager.defineId(Gurty.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> TRAVELLING = EntityDataManager.defineId(Gurty.class,
			DataSerializers.BOOLEAN);
	...
	private static int calcNetMobCap(IWorld level, BlockPos pos) {
		...
		int mobCap = MyConfig.getGurtySpawnCap() + 
	...
	public static boolean canSpawn(EntityType<? extends Gurty> gurtyIn, IWorld level, SpawnReason reason,
		...
		int mobCount = ((ServerWorld) level).getEntities(ModEntities.GURTY, (entity) -> true).size();
		if (mobCount >= calcNetMobCap(level, pos)) {
			return false;
		}

		Utility.debugMsg(1, "Gurty spawned at " + pos.getX() + " " + pos.getY() + " " + pos.getZ());
		return true;

	}
	
	@Nullable
	private static Vector3d findSolidBlock(EntityType<? extends Gurty> gurtyIn, IWorld world, BlockPos blockPos,
			int maxXZ, int maxY) {
...
public static boolean isBubbleColumn(IWorld world, BlockPos pos) {
	...
	private static boolean isWaterNearby(IWorld level, BlockPos pos) {
	...
	public Gurty(EntityType<? extends Gurty> type, World worldIn) {
	...
	protected void defineSynchedData() {
		...
		this.entityData.define((DataParameter<Integer>) Gurty.TARGET_ENTITY, 0);
		this.entityData.define((DataParameter<Boolean>) Gurty.ANGRY, false);
	...
	public LivingEntity getTargetedEntity() {
		...
		final Entity targetEntity = this.level
				.getEntity((int) this.entityData.get((DataParameter<Integer>) Gurty.TARGET_ENTITY));
	...
	public boolean hasTargetedEntity() {
		return (int) this.entityData.get((DataParameter<Integer>) Gurty.TARGET_ENTITY) != 0;
	}
...
public boolean isAngry() {
	return this.entityData.get((DataParameter<Boolean>) Gurty.ANGRY);
}
...
private boolean isGoingNest() {
	...
	public void setAngry(boolean bool) {
		this.entityData.set((DataParameter<Boolean>) Gurty.ANGRY, bool);
	}
	...
	private void setTargetedEntity(final int targetEntityId) {
		this.entityData.set((DataParameter<Integer>) Gurty.TARGET_ENTITY, targetEntityId);
	}
